...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create budget-app-761f
  fly apps create budget-app-761f-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app budget-app-761f
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app budget-app-761f-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'budget-app-761f-staging' does not match app name in config file 'budget-app-761f'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name budget-app-761f-db
  fly postgres attach --app budget-app-761f budget-app-761f-db

  fly postgres create --name budget-app-761f-staging-db
  fly postgres attach --app budget-app-761f-staging budget-app-761f-staging-db
  ```
